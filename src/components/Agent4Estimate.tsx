...
export const Agent4Estimate: React.FC = () => {
  ...
  const handleCustomSend = () => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes('insurance') || lower.includes('adjuster') || lower.includes('claim')) {
        response = `Yes, our supplement specialists specialize in working directly with carriers in the ${companySettings.city} area. We make sure they pay out the full amount needed for a code-compliant, safe roof replacement.`;
      } else if (lower.includes('pay') || lower.includes('afford') || lower.includes('finance') || lower.includes('cost')) {
        response = `We understand budget constraints! That's why we support: ${companySettings.financingOptions}. Let's discuss a plan that keeps your out-of-pocket costs at zero.`;
      } else {
        response = `Thank you for sharing that. At ${companySettings.companyName}, we are dedicated to providing the highest quality roofing solutions. Let's schedule a quick call with our estimator to resolve this and finalize your project schedule.`;
      }
      ...
      // Save follow-up interaction details into the lead profile if desired
          ...
          <div className="pt-4 border-t border-slate-800 shrink-0 space-y-3 mt-4">
            <div className="flex space-x-3">
              <input
                ...
                className="flex-1 bg-slate-950 border border-slate-850 rounded-xl px-4 py-2.5 text-xs text-slate-100 focus:outline-none focus:border-orange-500"
              />
              <button
                onClick={handleCustomSend}
                className="bg-orange-500 hover:bg-orange-600 px-4 py-2.5 rounded-xl text-slate-950 text-xs font-black transition"
              >
                Send Objections
              </button>
            </div>
