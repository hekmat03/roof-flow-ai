...
export const Agent3SMSEmail: React.FC = () => {
  ...
  const handleUserReply = () => {
    ...
    setSimulationLogs(prev => [...prev, userReply]);
    setUserReplyText('');

    // AI Nurture response generator
    setTimeout(() => {
      let aiResponseText = '';
      const lower = userReplyText.toLowerCase();

      if (lower.includes('yes') || lower.includes('ok') || lower.includes('sure') || lower.includes('book')) {
        aiResponseText = `Perfect! I can get that scheduled for tomorrow at 2:00 PM. Could you confirm the best contact phone number to sync with our CRM?`;
      } else if (lower.includes('price') || lower.includes('cost') || lower.includes('free')) {
        aiResponseText = `Our inspections are 100% free with absolutely zero obligation! We provide a full digital photo report for your records. Can we set it up for you?`;
      } else {
        aiResponseText = `Understood. We can schedule the inspection around your availability, or perform a contact-free drone inspection while you're at work! What option would work better for you?`;
      }
                  ...
                  <div className="pt-4 border-t border-slate-800 shrink-0 space-y-3">
                    <div className="flex space-x-2">
                      <input
                        ...
                        className="flex-1 bg-slate-950 border border-slate-850 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-orange-500"
                      />
                      <button
                        onClick={handleUserReply}
                        className="bg-orange-500 hover:bg-orange-600 px-4 rounded-xl text-slate-950 text-xs font-bold transition"
                      >
